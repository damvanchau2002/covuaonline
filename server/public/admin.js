...
// handleAction được định nghĩa ở cuối file với đầy đủ các hành động (kèm viewer)

function wire() {
  document.getElementById('refresh').addEventListener('click', loadOnce);
  document.getElementById('rooms').addEventListener('click', (ev) => {
    const target = ev.target;
    const btn = target.closest ? target.closest('button[data-act]') : null;
    if (!btn) return;
...
// ====== Viewer ======
let viewerState = { id: null, timer: null };
...
async function loadViewerOnce() {
  ...
  const data = await fetchJSON(`/api/rooms/${viewerState.id}`);
  const { fen, history, turn, id } = data;
  document.getElementById('viewerRoom').textContent = `Phòng: ${id}`;
  document.getElementById('viewerTurn').textContent = `Lượt: ${turn === 'w' ? 'Trắng' : 'Đen'}`;
  renderBoard(fen);
  const movesUl = document.getElementById('viewerMoves');
